...
def start_tcp_lang_server(bind_addr, port, check_parent_process, handler_class):
    if not issubclass(handler_class, MarkdownLanguageServer):
        raise ValueError('Handler class must be an instance of MarkdownLanguageServer')
...
class MarkdownLanguageServer(MethodDispatcher):
    ...
    def __getitem__(self, item):
        ...
        try:
            return super(MarkdownLanguageServer, self).__getitem__(item)
        except KeyError:
    ...
    def capabilities(self):
        server_capabilities = {
            ...
            #'experimental': merge(self._hook('mdls_experimental_capabilities'))
        }
    ...
    def m_initialize(self, processId=None, rootUri=None, rootPath=None, initializationOptions=None, **_kwargs):
        ...
        self._dispatchers = self._hook('mdls_dispatchers')
        self._hook('mdls_initialize')
    ...
    def m_initialized(self, **_kwargs):
        self._hook('mdls_initialized')

    def code_actions(self, doc_uri, range, context):
        return flatten(self._hook('mdls_code_actions', doc_uri, range=range, context=context))

    def code_lens(self, doc_uri):
        return flatten(self._hook('mdls_code_lens', doc_uri))

    def completions(self, doc_uri, position):
        completions = self._hook('mdls_completions', doc_uri, position=position)
        return {
            'isIncomplete': False,
    ...
    def format_document(self, doc_uri):
        return self._hook('mdls_format_document', doc_uri)

    def format_range(self, doc_uri, range):
        return self._hook('mdls_format_range', doc_uri, range=range)

    def highlight(self, doc_uri, position):
        return flatten(self._hook('mdls_document_highlight', doc_uri, position=position)) or None
    ...
    def references(self, doc_uri, position, exclude_declaration):
        return flatten(self._hook(
            'mdls_references', doc_uri, position=position,
            exclude_declaration=exclude_declaration
        ))

    def rename(self, doc_uri, position, new_name):
        return self._hook('mdls_rename', doc_uri, position=position, new_name=new_name)

    def signature_help(self, doc_uri, position):
        return self._hook('mdls_signature_help', doc_uri, position=position)

    def folding(self, doc_uri):
        return self._hook('mdls_folding_range', doc_uri)
    ...
    def m_workspace__did_change_watched_files(self, changes=None, **_kwargs):
        ...
        for d in (changes or []):
            if d['uri'].endswith(MARKDOWN_FILE_EXTENSIONS):
                changed_md_files.add(d['uri'])
